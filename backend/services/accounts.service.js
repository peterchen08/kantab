...
const FIELDS = {
	...
	*/
	...
	/**
	 * virtual:true 虚拟字段,在数据库中不存在,必须设置get()方法
	 * get函数在transforming entities时被调用,在返回前通过其他field字段计算出虚拟值
	 * get: ({ value }) => value.replace(/(\d{4}-){3}/g, "****-****-****-")
	 * 如果get的值是一个字符串,则调用method中定义对应函数名的方法
	 * email: { type: "string", set: value => value.toLowerCase() }
	 * set函数则在creating或者update的时候被调用
	 */
...
module.exports = {
	...
	settings: {
		/**
		 * rest、actions、fields、indexes都是针对@moleculer/database的设置
		 */

		// GET /{serviceName}/count 是否启用rest endpoint风格
		rest: true,

		/**
		 * 只要mixins的时候createActions参数不设置为false,都将service将自动生成CRUD actions
		 * mixins: [DbService({
            createActions: false
           })]
		 */
	...
	actions: {
		...
		disable: {
			...
			/**
			 * check-ermissions中间件读取下面这个属性判断操作权限
			 */
			permissions: [C.ROLE_SUPER_ADMIN],
			params: {
				id: { type: "string" }
	...
	methods: {
		 ...
		 */
		async getToken(user) {
			return await this.generateJWT({ id: user.id.toString(), type: C.TOKEN_TYPE_ACCOUNT_VERIFICATION });
		},
	 ...
	 */
	started() {
		this.logger.info("---------------accounts config-----------");
